...
printf "[org.kde.kdecoration2]\nlibrary=org.kde.breeze\ntheme=Breeze\n\n[Plugins]\nmindtile_wheelEnabled=true\n" > "$out/config/kwinrc"
# The test input tool and the Meta+wheel effect (make tools).
make -s -C "$here/.." tools >&2 || exit 1
mkdir -p "$out/plugins/kwin/effects/plugins"
cp "$here/../build/effect/mindtile_wheel.so" "$out/plugins/kwin/effects/plugins/"
...
timeout ${MT_TIMEOUT:-120} dbus-run-session -- env \
    ...
    QT_QPA_PLATFORM=wayland KWIN_SCREENSHOT_NO_PERMISSION_CHECKS=1 KWIN_WAYLAND_NO_PERMISSION_CHECKS=1 QT_PLUGIN_PATH="$out/plugins" \
